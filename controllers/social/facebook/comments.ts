...
export const facebook_comment_scraper = async (req: Request, res: Response) => {
    ...
    console.log(payload)
    try {
        const actorUrl = `https://api.apify.com/v2/acts/apify~facebook-comments-scraper/runs?token=${apiKey}`;
        ...
        const output_data = result.map((postData: any) => ({
            username: postData.profileId,
            created_at: postData.date,
        }));

        const usernames = output_data.map(i=>`https://www.facebook.com/profile.php?id=${i.username}`)
