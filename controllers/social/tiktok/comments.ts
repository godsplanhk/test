...
export const tiktok_comments_scraper = async (req: Request, res: Response) => {
    try {
        ...
        const actorUrl = `https://api.apify.com/v2/acts/apidojo~tiktok-comments-scraper/runs?token=${apiKey}`;

        // Start actor and poll in parallel
        const response = await fetch(actorUrl, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ startUrls:[url], maxItems:limit || 1 }),
        });
        ...
        const usernames = result.map(i=> i.user.username)
        const scrape_profile=async()=>{
            ...
            const response = await fetch(actorUrl, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ profiles:usernames, "profileSorting":"latest"}),
            });
            let data  = await response.json();
            console.log(data)
            data = data.data
